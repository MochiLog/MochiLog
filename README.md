...
ログの解析は端末内で行います。iCloud同期を有効にした場合は、記録を利用者のiCloudプライベートデータベースへ保存します。Watch利用時はペアリングしたWatchへ転送し、共有やお問い合わせでは利用者が選んだ情報を送信します。iOS/iPadOS 27・macOS 27向けMac連携ベータでは、Macがロック解除中の端末からログを収集し、ローカルネットワーク経由で暗号化してアプリへ転送します。問い合わせ時には、送信操作により端末・Macの診断情報がメールに添付されます。

開発者へバッテリー記録が自動送信される仕組みはありません。データの保存・削除、問い合わせ時に送る情報、外部サービスについては [プライバシーポリシー](MochiLog/Resources/PrivacyPolicy.md) と [利用規約](MochiLog/Resources/TermsOfUse.md) を参照してください。
